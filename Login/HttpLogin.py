...
class WiFlyLogger(Logger):
    ...
    """
    URL_ENTRY = 'https://www.wifly.com.tw/superwifi/4FreeLogin.aspx'
    URL_JUMP_PAGE = 'https://www.wifly.com.tw/superwifi/4FreeLogin.aspx'
    # URL_JUMP_PAGE_2 = 'http://pc.4free.com.tw/wifly/login_content'
    # URL_AD_LOG = 'http://pc.4free.com.tw/adlog'
    # URL_LOGIN = 'http://pc.4free.com.tw/postAPLogin'
    URL_LOGIN = 'https://wac.wifly.com.tw/login'
    ...
    def login(self):
        ...
        try:
            ...
            if resp.status == 200:
                ...
                print userInfo
                post_data = {'loginkey':userInfo['loginkey'],
                             'mac':userInfo['mac'],
                             'dsturl':'http://pc.4free.com.tw/postAPLogin'}
                resp, content = self.h.request(self.URL_ENTRY,
                                               method='POST',
                                               headers=headers,
                                               body=urlencode(post_data))
                ...
                #     print item.extract()
                #     adParams['id'] = item.xpath('@data-id').extract()[0]
                #     adParams['source'] = item.xpath('@data-source').extract()[0]
                #     adParams['type'] = 'view'
                #     adParams['kind'] = item.xpath('@data-kind').extract()[0]
                #     adParams['_csrf'] = item.xpath('@data-csrf').extract()[0]
                #     adParams['site_id'] = post_data['apname']
                # print json.dumps(adParams)
                # resp, content = self.h.request(self.URL_AD_LOG,
                #                                method='POST',
                #                                body=json.dumps(adParams),
                #                                headers=headers)
                # print resp
                # resp, content = self.h.request(self.URL_LOGIN)
                # print resp
...
class ITaiwanLogger2(Logger):
    ...
    def login(self):
        try:
            ...
            post_data = {self.KEY_USER_NAME: self.PRE_TAG_NAME+self.TPE_USERNAME+'.tpe' + self.POST_TAG_NAME,
                         self.KEY_PASSWORD: self.TPE_PASSWORD,
                         'clt_user':self.TPE_USERNAME,
                         'clt_pass':self.TPE_PASSWORD,
                         'type':'1',
                         'session':''}
            #
            resp, content = self.h.request(self.URL_LOGIN,
            ...
            # resp, content = self.h.request('http://wlangw.hinet.net/')
            print resp.status
            print resp
            print content
